---
title: "Changes of Dis Manibus inscriptions in time (text)"
author: 
...
# Exploration of the text of DM inscriptions
...
Tokenizing of dataset for individual centuries
```{r}
EDH_1_tokenized <- EDH_1CAD %>% 
  unnest_tokens(word, clean_text_interpretive_word, token = stringr::str_split, pattern = " ") %>% 
  drop_na(word) %>%
  print()

EDH_2_tokenized <- EDH_2CAD %>% 
  unnest_tokens(word, clean_text_interpretive_word, token = stringr::str_split, pattern = " ") %>% 
  drop_na(word) %>%
  print()

EDH_3_tokenized <- EDH_3CAD %>% 
  unnest_tokens(word, clean_text_interpretive_word, token = stringr::str_split, pattern = " ") %>% 
  drop_na(word) %>%
  print()
```
